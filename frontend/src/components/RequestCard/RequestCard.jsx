        ...
        <div className='request-card px-3 mb-3'>
            <div className={`position-relative d-flex ${type === 'earlier' ? "align-items-center" : "align-items-start"} gap-3`}>
                ...
                <div className="request-avatar">
                    {user ? <img className='user rounded-pill' src={`${UPLOAD_URL}/${user}`} alt="" /> : <img src={User} alt="" />}
                    {type === 'earlier' ? <img className='userIcon' src={UserIcon} alt="" /> : (type === 'notification' ? <img className='userIcon' src={Notification} alt="" /> : <img className='userIcon' src={UserIcon} alt="" />)}
                </div>
